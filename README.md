...
- order - объект, где будут храниться данные для оформления заказа.
...
- clearBasket() - очищает содержимое корзины после завершения заказа.
...
- clearOrder() - очищает все поля формы заказа после его завершения.
...
- set buttonToggler(items: string[]) - переключает состояние кнопки в зависимости наличия товаров в корзине.
...
- set total - сеттер для установки и отображения общей суммы заказа.
