...
- \_items - массив IProduct объектов для хранения.
...
- getItem(id: string) - принимает параметр типа string и возвращает данные об еденице товара по его id.
...
Конструктор в качестве параметров принимает элемент разметки(темплейт) будущего контейнера для вывода данных и объект для событий.\
...
- set price - сеттер принимает параметр типа number или null и устанавливает цену товара. В зависимости от параметра (number или null) устанавливает текст для отображения ("кол-во синапсов" или "Бесценно");
...
Конструктор в качестве параметров принимает элемент разметки будущего контейнера (темплейт) для вывода данных и объект для событий.\
...
Класс предназначен для отображения корзины заказов.\
Наследуется от базового класса Component.
...
- basketPrice - элемент разметки для отображения итоговой стоимости товаров в корзине;

Конструктор: 

Инициализирует поля класса.\
На конопку для оформления заказа(basketOrderButton) устанавливает слушатель, который при клике на кнопку  генерирует событие 'order:open'.
